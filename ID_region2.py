...
def pro(name,pic,w,h):
    ...
    for i in range(len(contours)):
        ...
        if(rect[1][0]>rect[1][1]):
            ...
            maxer=rect[1][1]
            miner=rect[1][0]
        if(maxer>=0.18*w and maxer<=0.30*w)and(miner>=0.04*h and miner<=0.09*h)and(((math.fabs(rect[2])>88) and(rect[1][0]<rect[1][1]))or((math.fabs(rect[2])<2)and(rect[1][0]>rect[1][1])) ):
